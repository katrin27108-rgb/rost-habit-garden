...
| Variable | `CLOUDFLARE_DEPLOY_ENABLED` | установите `true` только после добавления остальных настроек |
...
Пока `CLOUDFLARE_DEPLOY_ENABLED` отсутствует или не равно `true`, CI продолжает проверять `main`, но production deployment безопасно пропускается. Это позволяет подключить репозиторий до передачи инфраструктурных секретов.
...
Для уже существующей D1 новые колонки применяются отдельной одноразовой миграцией из `drizzle/`. Bootstrap-файл содержит полную схему для новых баз, но `CREATE TABLE IF NOT EXISTS` не добавляет колонки в существующую таблицу.
