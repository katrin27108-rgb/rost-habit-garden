...
flowchart LR
  ...
  Prototype --> Plants[PhysiologicalPlant.tsx: процедурные SVG-растения]
  Prototype --> DemoState[Локальное состояние пробной страницы]
  DemoState --> Growth[Непрерывный анатомический рост от каждого выполнения]
  Growth --> Anatomy[Корни, стебли, отдельные листья, бутоны, лепестки и плоды]
...
Маршрут `/living-garden` — изолированная пробная визуальная концепция. Он не меняет сохранённую модель привычек, API или активную 3D-сцену. В прототипе одна цель по-прежнему соответствует одному выбранному растению; декоративные взаимодействия не увеличивают прогресс. Каждый подтверждённый шаг меняет отдельные части SVG-растения, а не масштабирует цельную картинку. Подтверждение находится в одной карточке с растением, чтобы рост был виден в момент действия.
