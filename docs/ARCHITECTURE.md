...
flowchart LR
  ...
  Prototype --> Plants[public/plants/growth/вид/NN.png: независимые физиологические кадры]
  ...
  DemoState --> Checklist[Выбор растения и список выполненного]
  DemoState --> Confirm[Подтверждение привычки рядом с выбранным растением]
  DemoState --> Care[Доброе слово и удобрение: анимированный визуальный отклик]
```

Маршрут `/living-garden` — изолированная пробная визуальная концепция. Он не меняет сохранённую модель привычек, API или активную 3D-сцену. В прототипе одна цель по-прежнему соответствует одному выбранному растению; декоративные взаимодействия не увеличивают прогресс. Каждый вид имеет 16 последовательных физиологических кадров — от семени и ростка до зрелой формы. Кадры хранятся отдельными прозрачными изображениями с безопасными полями. В покое отображается ровно один кадр, а подтверждение действия сначала гасит прежний и только затем показывает новый. Поэтому изображения не накладываются, соседние стадии не попадают в область просмотра, а листья не обрезаются. Между физиологическими кадрами малое изменение масштаба отражает промежуточный рост каждого шага; взрослое растение не используется как универсальная заготовка. Подтверждение находится в одной карточке с растением, чтобы рост был виден в момент действия.
