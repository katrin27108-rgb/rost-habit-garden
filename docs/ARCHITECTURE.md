...
flowchart LR
  ...
  Prototype --> Plants[public/plants/*.webp: пастельные иллюстрации растений]
  Prototype --> DemoState[Локальное состояние пробной страницы]
  DemoState --> Growth[Плавное проявление одной иллюстрации от каждого выполнения]
...
Маршрут `/living-garden` — изолированная пробная визуальная концепция. Он не меняет сохранённую модель привычек, API или активную 3D-сцену. В прототипе одна цель по-прежнему соответствует одному выбранному растению; декоративные взаимодействия не увеличивают прогресс. Каждый подтверждённый шаг плавно открывает следующий фрагмент одной исходной иллюстрации снизу вверх; картинка не масштабируется и не заменяется другой стадией. Подтверждение находится в одной карточке с растением, чтобы рост был виден в момент действия.
