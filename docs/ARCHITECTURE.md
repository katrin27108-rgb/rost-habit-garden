...
flowchart LR
  ...
  Prototype --> Plants[public/plants/growth/*-growth.png: 4×4 листы физиологических стадий]
  Prototype --> DemoState[Локальное состояние пробной страницы]
  DemoState --> Growth[Плавное смешивание соседних кадров от каждого выполнения]
...
Маршрут `/living-garden` — изолированная пробная визуальная концепция. Он не меняет сохранённую модель привычек, API или активную 3D-сцену. В прототипе одна цель по-прежнему соответствует одному выбранному растению; декоративные взаимодействия не увеличивают прогресс. Каждый вид имеет 16 последовательных физиологических кадров — от семени и ростка до зрелой формы. Прогресс непрерывно смешивает соседние кадры, а подтверждение действия анимирует переход от прежнего состояния к новому. Поэтому изменение видно после каждого шага без масштабирования готового взрослого растения и без проявления его фрагментами. Подтверждение находится в одной карточке с растением, чтобы рост был виден в момент действия.
