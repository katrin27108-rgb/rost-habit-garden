...
Без входа привычки сохраняются на устройстве. IndexedDB хранит очередь изменений для повторной отправки. После регистрации или входа API синхронизирует сад через Cloudflare D1. Пробная `/living-garden` использует тот же аккаунт и отдельный серверный снимок: даты выполнений с разных устройств объединяются. Таблицы описаны в `db/schema.ts`, а production-инициализация — в `drizzle/cloudflare-bootstrap.sql` и миграциях `drizzle/`.
